...
func TestSnapshotViewPaging(t *testing.T) {
    ...
    // Verify data length and sorting by Revision ascending via List
    list, err := sv.List("")
    if err != nil {
        t.Fatalf("List returned error: %v", err)
    }
    if len(list) != 3 {
        t.Fatalf("expected 3 items in SnapshotView, got %d", len(list))
    }
